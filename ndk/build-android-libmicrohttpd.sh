...
#BUILD_ARCHS="arm64-v8a,x86"
BUILD_API=21
BUILD_PIE=1
BUILD_SQLCIPHER=0

SRCDIR=libmicrohttpd-0.9.55
...
for android_arch in $BUILD_ARCHS; do
  ...
  #export GNUTLS_CFLAGS=`pkg-config gnutls --cflags`
  #export GNUTLS_LIBS=`pkg-config gnutls --libs`

  #echo $GNUTLS_CFLAGS, $GNUTLS_LIBS
  ...
  ./configure --host="$gcc_prefix" --disable-shared --enable-static --enable-https --disable-curl --disable-examples --disable-doc --prefix="$android_prefix" CFLAGS="$android_cflags" LDFLAGS="$android_libs" --with-libgcrypt-prefix="$android_prefix" --with-gnutls="$android_prefix" LIBS="-lnettle -lhogweed -ltasn1 -lidn2 -lgmp -lunistring -liconv -lz"
