...
#OPENSSL_CONFIG="no-rc2 no-cast \
#  no-md2 no-md4 \
#  no-shared no-comp no-hw no-engine no-zlib \
#  -ffunction-sections -fdata-sections -Wl,--gc-sections"
...
USE_CLANG=0
BUILD_ARCHS="armeabi-v7a,arm64-v8a,x86"
BUILD_API=21
BUILD_PIE=1
...
for android_arch in $BUILD_ARCHS; do
  ...
  android_cflags="$android_cflags -Wall -funwind-tables -fstack-protector -fomit-frame-pointer"
  android_cppflags=" -I$android_prefix/include -DNDEBUG -DANDROID"
  android_libs="-L$android_prefix/lib"
  ...
  ./configure --host="$gcc_prefix" --disable-shared --enable-static --prefix="$android_prefix" --disable-debug --enable-optimize --disable-ftp --disable-ldap --disable-file --disable-rtsp --enable-proxy --disable-dict --disable-telnet --disable-tftp --disable-pop3 --disable-imap --disable-smb --disable-smtp --disable-gopher --disable-manual --disable-ipv6 --disable-sspi --disable-ntlm-wb --disable-threaded-resolver --disable-cookies --without-librtmp --without-libssh2 --disable-unix-sockets --without-nghttp2 CFLAGS="$android_cflags" CPPFLAGS="$android_cppflags" LDFLAGS="$android_libs" # LIBS=" -lcrypto -lssl" 
  #./Configure $openssl_conf $android_cflags $OPENSSL_CONFIG \
  #  --openssldir="$android_prefix" --prefix="$android_prefix" || exit 1
