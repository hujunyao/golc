...
for android_arch in $BUILD_ARCHS; do
  ...
  android_cflags="$android_cflags -I$android_prefix/include -Wall -funwind-tables -fstack-protector -fomit-frame-pointer -DNDEBUG -DANDROID"
  ...
  #./configure --host="$gcc_prefix" --disable-shared --enable-static --prefix="$android_prefix" CFLAGS="$android_cflags" LDFLAGS="$android_libs"
  cmake -DCMAKE_SYSTEM_NAME=Android -DCMAKE_SYSTEM_VERSION=$android_api -DCMAKE_ANDROID_ARCH_ABI=$android_arch -DCMAKE_ANDROID_NDK=$ANDROID_NDK_ROOT -DENABLE_TESTING=off -DCMAKE_INSTALL_PREFIX=$android_prefix  -DCMAKE_BUILD_TYPE=Release -DOPENSSL_ROOT_DIR=$android_prefix -DWITH_GCRYPT=on -DWITH_PCAP=off -DWITH_TESTING=off -DWITH_CLIENT_TESTING=off -DWITH_INTERNAL_DOC=off -DCMAKE_SYSROOT=$android_sysroot -DWITH_STATIC_LIB=on ..
