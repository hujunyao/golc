...
for android_arch in $BUILD_ARCHS; do
  ...
  cd $SRCDIR
  #ln -s syscfg/lock-obj-pub.aarch64-unknown-linux-gnu.h  syscfg/lock-obj-pub.aarch64-unknown-linux-android.h
  #ln -s syscfg/lock-obj-pub.i686-pc-linux-gnu.h  syscfg/lock-obj-pub.i686-pc-linux-android.h
  [ -f 'Makefile' ] && make distclean
  CC="$android_cc" \
  AR="$gcc_prefix-ar" \
  RANLIB="$gcc_prefix-ranlib" \
  ./configure --host="$gcc_prefix" --disable-doc --disable-languages --disable-tests --disable-shared --disable-tests --enable-static --prefix="$android_prefix" CFLAGS="$android_cflags" LDFLAGS="$android_libs"
