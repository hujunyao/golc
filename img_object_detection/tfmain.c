...
const char* classes_name[] = {"dummy", "earsoup", "rice", "diamond", "lacery", "octagon", "triangle", "small_shellw", "big_shell", "rect"};
...
TF_Tensor* create_image_tensor_from_v4l2(const char* jpegfn, float* img_width, float* img_height) {
  ...
  if(jpegBuf != NULL) {
    if(tjDecompressHeader3(tjInstance, jpegBuf, jpegSize, &width, &height, &inSubsamp, &inColorspace) < 0) {
      DD("%s isn't valid jpeg file, error = %s\n", jpegfn, tjGetErrorStr2(tjInstance));
    }
    DD("%s size = %d, width = %d, height = %d, tjInstance = %p\n", jpegfn, jpegSize, width, height, tjInstance);
    *img_width = (float) width, *img_height = (float) height;
    ...
    if(tjDecompress2(tjInstance, jpegBuf, jpegSize, imgBuf, width, 0, height, TJPF_RGB, 0) < 0) {
      DD("decompress %s as TJPF_RGB failed\n", jpegfn);
    }
...
void show_tf_devices(TF_Graph* graph) {
  ...
  if(TF_GetCode(status) != TF_OK) {
    DD("show_tf_devices.TF_SessionListDevices failed\n");
  } else {
    int ndev = TF_DeviceListCount(devices);
    DD("tensorflow device number %d\n", ndev);
    for(int i = 0; i < ndev; ++i) {
      DD("device.%d name = %s, type = %s\n", i, 
...
int image_detection_setup(gotensor_t* ptr) {
  ...
  DD("image_detection_setup time = %lld msec\n\n", etm-stm);
...
static void draw_tfoutput_for_imgbuf(gotensor_t* go, cairo_t* cr, int output_idx, int classes, float scores, float x0, float y0, float x1, float y1) {
  ...
  cairo_close_path (cr);
  cairo_set_source_rgba (cr, 0.6, 0, 0, scores);
  ...
  snprintf(scores_text, 256, "T%d.scores %.2f", output_idx, scores);
  ...
  ty = y1 + (extents.height + extents.y_bearing) - line_width;
  ...
  //  DD("cairo_image_surface_write_to_jpeg failed, e = %x\n", e);
  //}
...
void do_rgbimg_detection(gotensor_t* ptr, unsigned char* rgbbuf, int nbytes, int img_width, int img_height) {
  ...
  if(ptr != NULL) {
    ...
    DD("do_rgbimg_detection, v4l2_tensor = %p, tfSession = %p, tfHandle = '%s'\n", v4l2_tensor, ptr->tfSession, ptr->tfHandle, ptr->tfHandle);
    if(v4l2_tensor != NULL) {
      int ret = 0;
      int count = 0;
      ...
      DD("do_rgbimg_detection.TF_SessionPRun status %c= TF_OK (%s), tm = %ld\n", ret ? '!' : '=', TF_Message(status), time(NULL));
      ...
      //  case 1:
      //  DD("oo_tensor[%d].name = %s (%d)", i, tensor_name[i], TF_Dim(t, 0));
      //  break;
      //  case 2:
      //  DD("oo_tensor[%d].name = %s (%d, %d)", i, tensor_name[i], TF_Dim(t, 0), TF_Dim(t, 1));
      //  break;
      //  case 3:
      //  DD("oo_tensor[%d].name = %s (%d, %d, %d)", i, tensor_name[i], TF_Dim(t, 0), TF_Dim(t, 1), TF_Dim(t, 2));
      //  break;
      //  case 4:
      //  DD("oo_tensor[%d].name = %s (%d, %d, %d, %d)", i, tensor_name[i], TF_Dim(t, 0), TF_Dim(t, 1), TF_Dim(t, 2), TF_Dim(t, 3));
      //  break;
      //  }
      //  DD(" type = %x, bytesize = %d\n", TF_TensorType(t), nbytes);
      //}
      ...
      for(int i = 0; i < count; i++, scores_data++, classes_data++) {
        ...
        if(scores > 0.3f) {
          ...
          if(i > 0) {
            oojson += sprintf(oojson, ",{\"outputClasses\":%d,\"outputScores\":%.2f,\"x0\":%.2f,\"y0\":%.2f,\"x1\":%.2f,\"y1\":%.2f}", classes, scores, x0, y0, x1, y1);
          } else {
            oojson += sprintf(oojson, "{\"outputClasses\":%d,\"outputScores\":%.2f,\"x0\":%.2f,\"y0\":%.2f,\"x1\":%.2f,\"y1\":%.2f}", classes, scores, x0, y0, x1, y1);
          }
  ...
  //gettimeofday(&tm, NULL);
  //etm = tm.tv_sec * 1000LL + tm.tv_usec/1000;
  //DD("do_rgbimg_detection time = %lld msec\n\n", etm-stm);
  //DD("output=%s\n", ptr->output_json);
}
...
void do_jpegimg_detection_ex(gotensor_t* ptr, const char* jpegfn) {
  ...
  if(ptr != NULL) {
    ...
    DD("do_jpegimg_detection_ex, v4l2_tensor = %p, tfSession = %p, tfHandle = %p '%s'\n", v4l2_tensor, ptr->tfSession, ptr->tfHandle, ptr->tfHandle);
    if(v4l2_tensor != NULL) {
      int ret = 0;
      int count = 0;
      float *boxes_data, *scores_data, *classes_data, *count_data;
      TF_SessionPRun(ptr->tfSession, ptr->tfHandle, feeds, in_tensors, 1, fetches, oo_tensor, 4, NULL, 0, status);
      ret = TF_GetCode(status) != TF_OK;
      DD("do_jpegimg_detection_ex.TF_SessionPRun status %c= TF_OK (%s), tm = %ld\n", ret ? '!' : '=', TF_Message(status), time(NULL));
  ...
  gettimeofday(&tm, NULL);
  etm = tm.tv_sec * 1000LL + tm.tv_usec/1000;
  DD("do_jpegimg_detection_ex %s time = %lld msec\n\n", jpegfn, etm-stm);
  DD("output = %s\n", ptr->output_json);
}
...
void do_image_detection(TF_Graph* graph,TF_Session* session, const char* jpegfn) {
  ...
  //DD("oo_scores = %p, oo_boxes = %p, oo_classes = %p, oo_count = %p, devicename = %s\n", oo_scores, oo_boxes, oo_classes, oo_count, TF_OperationDevice(oo_scores));

  DD("do_image_detection %s\n", jpegfn);
  ...
  if(v4l2_tensor != NULL) {
    ...
    ret = TF_GetCode(status) != TF_OK;
    DD("TF_SessionRun status %c= TF_OK (%s), tm = %ld\n", ret ? '!' : '=', TF_Message(status), time(NULL));
    boxes_data = TF_TensorData(oo_tensor[0]), scores_data = TF_TensorData(oo_tensor[1]), classes_data = TF_TensorData(oo_tensor[2]), count_data = TF_TensorData(oo_tensor[3]);
    count = (int) *(float *)count_data;
    DD("oo_tensor.data (%p, %p, %p, %p), num_detections = %d\n", boxes_data, scores_data, classes_data, count_data, count);
  ...
  gettimeofday(&tm, NULL);
  etm = tm.tv_sec * 1000LL + tm.tv_usec/1000;
  DD("do_image_detection %s time = %lld msec\n\n", jpegfn, etm-stm);
...
void dump_all_operation(TF_Graph* graph) {
  ...
  while ((oper = TF_GraphNextOperation(graph, &pos)) != NULL) {
    DD("OP.%d: op.name = %s, op.type = %s, op.device = %s, op.output.num = %d\n", pos, TF_OperationName(oper), TF_OperationOpType(oper), TF_OperationDevice(oper), TF_OperationNumOutputs(oper));
  }
}

int usbcamera_open(void) {
  int fd = 0;
  struct stat st;

  if(stat(USB_CAMERA_DEV, &st) == -1) {
    DD(" USB CAMERA %s isn't available\n", USB_CAMERA_DEV);
  }
  if (!S_ISCHR(st.st_mode)) {
    DD(" %s isn't USB CAMERA\n", USB_CAMERA_DEV);
  }

  fd = v4l2_open(USB_CAMERA_DEV, O_RDWR /* required */ | O_NONBLOCK, 0);
  DD("usbcamera_open.v4l2_open fd = %d\n", fd);
...
static void usbcamera_start(gotensor_t* go) {
  ...
  for (i = 0; i < go->n_buffers; ++i) {
      ...
      DD("usbcamera_start.xioctl VIDIOC_QBUF failed for index(%d)\n", i);
    }
  }
  
  type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
  if (-1 == xioctl(go->vfd, VIDIOC_STREAMON, &type)) {
    DD("xioctl VIDIOC_STREAMON failed\n");
  }
  DD("xioctl VIDIOC_STREAMON n_buffers = %d\n", go->n_buffers);
}
...
static void usbcamera_stop(int vfd) {
  ...
  if (-1 == xioctl(vfd, VIDIOC_STREAMOFF, &type)) {
    DD("xioctl VIDIOC_STREAMOFF failed\n");
  }
  DD("xioctl VIDIOC_STREAMOFF\n");
}
...
static int v4l2_mmap_init(gotensor_t* go) {
  ...
  if (-1 == xioctl(go->vfd, VIDIOC_REQBUFS, &req)) {
      ...
      DD("v4l2_mmap_init.xioctl VIDIOC_REQBUFS error = %s\n", strerror(errno));
  	}
    ret = -1;
  }

  DD("VIDIOC_REQBUFS req.count = %d\n", req.count);
  if (req.count < 2) {
  ...
  for (idx = 0; idx < req.count; ++idx) {
      ...
      DD("v4l2_mmap_init.xioctl VIDIOC_QUERYBUF failed\n");
    }
      ...
      DD("v4l2_mmap_init.v4l2_mmap failed for buffers[%d], buf.length = %d\n", idx, buf.length);
    }
  }
...
static int usbcamera_setup(gotensor_t* go) {
  ...
  if (0 == xioctl(go->vfd, VIDIOC_CROPCAP, &cropcap)) {
      ...
      //DD("xioctl VIDIOC_S_CROP errno = %s\n", strerror(errno));
  	}
  } else {
  	/* Errors ignored. */
  }
  ...
  DD("USB Camera accept format V4L2_PIX_FMT_YUV420, width = %d, height = %d \n", fmt.fmt.pix.width, fmt.fmt.pix.height);
...
void usbcamera_close(gotensor_t* go) {
  for (int i = 0; i < go->n_buffers; ++i) {
    if (-1 == v4l2_munmap(go->buffers[i].start, go->buffers[i].length)) {
      DD("usbcamera_close.v4l2_munmap failed fo buf[%d]\n", i);
    }
  }
  if(go->yuvimg != NULL) {
    free(go->yuvimg);
...
static void imageProcess(void* p, int width, int height, struct timeval timestamp) {
  ...
  if(dst != NULL) {
    ...
    if(access(bmpfn, F_OK) != 0) {
      ...
      DD("%s is exist, ignore this image frame\n", bmpfn);
    }
...
static void _uc_read_cb(evutil_socket_t fd, short event, void *arg) {
  ...
  DD(" _uc_read_cb event = %x, fd = %d\n", event, fd);
  if(event & EV_READ) {
    buf.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
    buf.memory = V4L2_MEMORY_MMAP;

    if (-1 == xioctl(fd, VIDIOC_DQBUF, &buf)) {
      DD("_uc_read_cb *xioctl VIDIOC_DQBUF error = %s errno = %x, EIO = %x, EAGAIN = %x\n", strerror(errno), errno, EIO, EAGAIN);
      if(errno == EAGAIN) {
        return;
      }
    }
    ...
    DD("VIDIOC_DQBUF buf.index = %d, n_buffers = %d\n", buf.index, go->n_buffers);
    
    if (-1 == xioctl(fd, VIDIOC_QBUF, &buf)) {
    	DD("_uc_read_cb **xioctl VIDIOC_QBUF error = %s", strerror(errno));
    }
  }
}
...
static unsigned char* do_yuvimg_detection (gotensor_t* go, int width, int height, unsigned long* jpegSize) {
  ...
  if(dst != NULL) {
    ...
    yuv420_rgb24_sseu(width, height, base_py, base_pu, base_pv, width, width/2, dst, nstride, YCBCR_JPEG);
    ...
    //tjSaveImage("/tmp/001.bmp", dst, width, nstride, height, TJPF_RGB, 0);
    {
      ...
      if(ret < 0) {
        DD("Compress RGB Image Buffer to JPEG Buffer failed\n");
      }
      tjDestroy(tjInstance);
  ...
  gettimeofday(&tm, NULL);
  etm = tm.tv_sec * 1000LL + tm.tv_usec/1000;
  DD("do_yuvimg_detection time = %lld msec\n\n", etm-stm);
  return jpegBuf;
}
...
static int gotensor_get_detection (void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *version, const char *upload_data, size_t *upload_data_size, void **ptr) {
  ...
  *ptr = NULL;                  /* reset when done */
  DD("gotensor_get_detection url = %s, method = %s, version = %s\n", url, method, version);
  ...
  if(strcmp(url, "/get/tensorflow/camera0") == 0) {
    ...
    if(jpegBuf != NULL) {
      ...
      if(fp != NULL) {
        ...
        fclose(fp);
        DD("Write USB Camera image to %s\n", myfn);
        //if(go->db != NULL) {
    ...
    DD("Write tensorflow output to %s\n", myfn);
...
static void gotensor_exec(gotensor_t* go) {
  ...
  for (;;) {
    ...
    //DD("gotensor_exec.select.r = %d, fd = %d, errno = %x, EINTR = %x\n", r, go->vfd, errno, EINTR);
  	if (EINTR == errno) {
      usleep(200);
      continue;
    }

    {
      ...
      if(buf.index < go->n_buffers) {
        //DD("buf.index = %d, go->n_buffers = %d, buf.length = %d\n", buf.index, go->n_buffers, go->buffers[buf.index].length);
        //imageProcess(go->buffers[buf.index].start, go->image_width, go->image_height, buf.timestamp);
        pthread_mutex_lock(&go->mutex);
        memcpy(go->yuvimg, go->buffers[buf.index].start, go->buffers[buf.index].length);
        //DD("cache yuv420 image\n");
        pthread_mutex_unlock(&go->mutex);
      }
      //DD("VIDIOC_DQBUF buf.index = %d, n_buffers = %d\n", buf.index, go->n_buffers);
...
int main (int argc, char* argv[]) {
  ...
  DD("tensorflow v%s, CUDA versioncode %d\n", TF_Version(), cudaver);

  const char* pbfn = argv[1];
  gotensor_t go = {0};

  if(CUDA_SUCCESS != cuInit(0)) {
    DD("CUDA initialize failed\n");
  }
  ...
  TF_Buffer* graphdef = load_graph_file(pbfn);
  if(graphdef != NULL) {
    TF_GraphImportGraphDef(graph, graphdef, opts, status);
    if(TF_GetCode(status) != TF_OK) {
      DD("import graph %s failed\n", pbfn);
    }
    DD("load graph file %s successful\n", pbfn);

    gotensor_new(&go, graph);

    //DO TF TEST 
    image_detection_setup(&go);
    //do_jpegimg_detection_ex(&go, "/tmp/001.jpg");

    //image_detection_setup(&go);
    //do_jpegimg_detection_ex(&go, "/tmp/002.jpg");

    //image_detection_setup(&go);
    //do_jpegimg_detection_ex(go, "/tmp/003.jpg");

    //do_image_detection(graph, session, argv[2]);
    //do_image_detection(graph, session, "/tmp/001.jpg");
    //do_image_detection(graph, session, "/tmp/003.jpg");
    //do_image_detection(graph, session, "/tmp/002.jpg");
