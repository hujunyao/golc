...
JNIEXPORT void JNICALL Java_com_skymobi_flashplayer_FlashplayerActivity_globalInit(JNIEnv *env, jobject obj, jobject this) {
  g_env = env;
  g_obj = (jobject) (*env)->NewGlobalRef(env, this);
  //g_obj = this;
  //g_obj = (jobject) (*env)->NewGlobalRef(env, g_obj);
  LOGE("globalInit, g_env = %x, g_obj = %x\n", g_env, g_obj);
  if(freetype_glue_init() < 0) {
    LOGE("freetype glue init failed\n");
  }
}

JNIEXPORT void JNICALL Java_com_skymobi_flashplayer_FlashplayerActivity_globalUninit(JNIEnv *env, jobject obj) {
  jboolean b = (*env)->IsSameObject(env, g_obj, obj);
  LOGE("IsSameObject = %d\n", b);

  freetype_glue_uninit();
}

JNIEXPORT void JNICALL Java_com_skymobi_flashplayer_PlasmaView_renderPlasma(JNIEnv * env, jobject  obj, jobject bitmap,  jlong  time_ms)
{
    ...
    skia_draw_text(pixels, info.width, info.height, 100, 100);
    skia_draw_rect(pixels, info.width, info.height, 10, 10, 100, 100);
...
JNIEXPORT void JNICALL Java_com_skymobi_flashplayer_PlasmaView_JNIonTouchEvent(JNIEnv *env, jobject obj, jfloat x, jfloat y, jfloat pressure, jfloat size) {
  LOGE("JNIonTouchEvent x = %f, y = %f\n", x, y);

  if(x > 80 && x < 100) {
    mrc_editCreate(NULL, "Hello From NDK", 0, 0);
  }
}

JNIEXPORT jboolean JNICALL Java_com_skymobi_flashplayer_PlasmaView_JNIonKeyDown(JNIEnv *env, jobject obj, jint keyCode, jobject ke) {
  LOGE("JNIonKeyDown keyCode = %x\n", keyCode);
  return 1;
}

JNIEXPORT jobject JNICALL Java_com_skymobi_flashplayer_PlasmaView_JNIopenFile(JNIEnv *env, jobject obj, jstring filename) {
  const jbyte *utf8 = (*env)->GetStringUTFChars(env, filename, NULL);
  if(utf8) {
    LOGE("JNIopenFile: filename = %s", utf8);
    (*env)->ReleaseStringUTFChars(env, filename, utf8);
  }
  jclass clazz = (*env)->FindClass(env, "com/skymobi/flashplayer/FlashplayerProp");
  if(clazz == 0) {
    LOGE("Find Class failed");
    return NULL;
  }
  jobject _obj = (*env)->AllocObject(env, clazz);
  jfieldID frame_rate = (*env)->GetFieldID(env, clazz, "FrameRate", "I");
  if(frame_rate == NULL) {
    LOGE("env->GetFieldID FrameRate failed");
    return _obj;
  }
  (*env)->SetIntField(env, _obj, frame_rate, 35);
